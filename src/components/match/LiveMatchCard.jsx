...
function LiveMatchCard({team1, team2, match, stadium, series}) {
  // console.log(team1)
  // console.log(team2)

    let matchTime = new Date(match.startTime)
    let matchDate = matchTime.toLocaleDateString("en-US", {dateStyle:"medium"})
      ...
      <div className="row no-gutters align-items-center">
        <div className="col-sm-12">
          <div className="detail-scorecard-head d-flex align-items-center justify-content-between">
            ...
            >
              <span className="detail-scorecard-match">{match.title}</span> •
              <span className="detail-scorecard-event">{series.shortName}</span> •
              <span className="detail-scorecard-venue">{stadium.shortName}</span> •
              <span className="detail-scorecard-venue">{matchDate}</span>
            </div>
            <div
              className={`detail-scorecard-head-format ${format_map[match.format]} d-flex align-items-center justify-content-end font-weight-bold`}
              style={{ width: "15%" }}
            >
        ...
        <div className="col-sm-12">
          <div className="detail-scorecard-match-status-text border-top d-flex align-items-center justify-content-between text-truncate">
            {
              match.statusInfo.replace("{{MATCH_START_HOURS}}", MATCH_START_HOURS).replace("{{MATCH_START_MINS}}", MATCH_START_MINUTES)
            }
          </div>
        </div>
      </div>
